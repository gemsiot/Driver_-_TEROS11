...
class TEROS11 final: public SDI12TalonSensor
{
	...
	constexpr static uint32_t DPS368_READ_ERROR = 0x80010000; //FIX! Error subtype = 1 for temp read, subtype = 2 for pres
	constexpr static uint32_t SHT3X_NAN_ERROR = 0x10040000; //FIX! Error subtype = 1 for temp read, subtype = 2 for RH
	constexpr static uint32_t DPS368_INIT_ERROR = 0x10010000; //FIX! Error subtype = I2C error code
	constexpr static uint32_t SHT3X_INIT_ERROR = 0x10030000; //FIX!
	constexpr static uint32_t SHT3X_I2C_ERROR = 0x10020000; //FIX! Error subtype = I2C error code

	public:
		TEROS11(SDI12Talon& talon_, uint8_t sensorPort_);
		...
		String getErrors();
		const char* name() override { return "METER Soil"; }
